...
proc logistic data = spdtmp7.VB_WB_LS_Alldata16_Train plots=ROC  
     ...
     descending  ;
model  Bought_flag = &variables_check. /  lackfit rsquare ctable outroc=ROC_sd;
...
if pred_response >= 0.019 then prediction = 1;
else prediction = 0;
run;
...
proc contents data= spdtmp7.VB_WB_LS_Alldata16_Train_res;
run;
/* Lift calculation */
